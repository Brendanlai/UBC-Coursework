...
if __name__ == "__main__":
    ...
    # Create and start processes for each row and append to processList to join
    for x in range(SIZE):
        process = multiprocessing.Process(target=checkRow, args=(testcase, x))
        process.start()

        processList.append(process)

    # Create and start processes for each column and append to processList to join
    for x in range(SIZE):
        process = multiprocessing.Process(target=checkColumn, args=(testcase, x))
        process.start()

        processList.append(process)

    # Create and start processes for each subgrid and append to processList to join
    for x in range(SIZE):
        process = multiprocessing.Process(target=checkSubgrid, args=(testcase, x))
        process.start()
