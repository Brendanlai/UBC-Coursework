$modde0cv
	...
	x: 	ds 4 ; 32-bits for var 'x'
	y:	ds 4 ; 32-bits for var 'y'
	bcd:ds 5 ; 10-digit packed BCD (each byte contains 2 digits)
	operation: ds 1
...
AddorMul:
	jb KEY.3, SubOrDiv 			; If '+' key not pressed, skip & check "-" key
	...
	jnb KEY.0, is_mult			; if key0 is pressed switch to mutliplication (i.e. go to is_mult
	mov operation, #0000_0001B
	jnb KEY.1, equal 			; If "=" key pressed, jump to equal
	lcall ReadNumber
	jnc toMain 					; Indirect jump to 'main loop'
	lcall Shift_Digits
	lcall Display 
	ljmp main 
is_mult:
...
SubOrDiv:
	jb KEY.2, equal 			; If '-' key not pressed, skip & check "=" key
	...
	lcall hex2bcd 				; Convert binary x to BCD
	lcall Display 				; Display the new BCD number
	jnb KEY.0, is_div			; if key0 is pressed switch to division (i.e. go to is_div)
...
toMain:
	ljmp main 					; 'forever' is to far away, need to use ljmp
...
; IMPLEMENTING MATH32 CALC FUNCTIONS
do_add:
	lcall add32
	lcall hex2bcd
	lcall display
	ljmp main
; Cannot handle negatives numbers, result must be positive	
do_sub:
	lcall xchg_xy				; changes x and y
	lcall sub32	
	lcall hex2bcd
	lcall display
	ljmp main
